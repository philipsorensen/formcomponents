...
<div class="{{ $col }} {{ config('formcomponents.div.padding') }}">
	...
	<input class="form-control @error($id) {{ config('formcomponents.is-invalid-class') }} @enderror" dir="rtl" id="{{ $id }}" name="{{ $id }}" type="number" {{ $attributes(['placeholder' => $name, 'value' => old($id)]) }} >
